...
このリポジトリは UTC、WG2、IRG の Document Registry と Unicode Public Review Issues (PRI) を入口に、Unicode 標準化の議論を LLM が読み、要約し、相互リンク付きの wiki として維持するためのものです。

Jxck/tc39-llm-wiki の考え方に合わせて、raw source、wiki、schema を分離します。ただしこのリポジトリでは、外部取得リソースの実体は Git に入れません。コミットするのは目録と wiki だけです。

## レイヤ

- **Remote registries** - `unicode.org` 上の公開 Document Registry と PRI。UTC、WG2、IRG、PRI の文書番号、表題、source / originator、日付、文書 URL の一次入口。
- **Catalog** - `catalog/registries/`。Registry / PRI から抽出した目録。Git にコミットしてよい。
...
```text
config/
  registries.json                 Document Registry / PRI の起点 URL と抽出設定
schema/
...
catalog/
  registries/
    ...
    irg/registers.json
    pri/documents.jsonl           Unicode Public Review Issues 由来の目録
    pri/registers.json
wiki/
...
- PRI: `PRI #546` のように表記する。ページ slug は `pri-546`。
...
Document Registry / PRI から目録を更新する。文書実体は取得しない。
...
オンライン同期では Registry の root / latest page を毎回再取得する。`--offline` はキャッシュだけで再現する場合、`--refresh` は過去 register も含めてすべて再取得する場合に使う。
