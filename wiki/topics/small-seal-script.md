...
| 2025-06 | WG2 | `WG2 N5304` | WG2 #72 Recommendation M72.13 が 11,339 Small Seal characters を `38000..3AC4A` に将来版候補として検討するよう勧告。 |
...
| 2026-06 | WG2 | `WG2 N5354` | WG2 #73 Recommendation M73.01 が CD4 disposition の一部として Small Seal character names を `SMALL SEAL CHARACTER-XXXXX` に変更し、26 characters の property correction を含める。M73.04 は 7th edition DIS への進行を勧告。 |
...
初期整理では variant set と重複 source の扱いが大きな論点だった。`WG2 N5306` は最終 proposal 前に約 450 要素の review が必要とし、`WG2 N5317` は TCA / China feedback に基づいて actionable dispositions を作った。N5317 時点では 199 characters を repertoire に追加する disposition となり、WG2 #72 の M72.13 では 11,339 characters が将来版候補として勧告された。
...
`WG2 N5344` では script property と character names は ISO 15924 に合わせて `Seal` を使う案だったが、WG2 #73 の M73.01 では、CD4 disposition の significant changes として character names を `SEAL CHARACTER-XXXXX` から `SMALL SEAL CHARACTER-XXXXX` に変更することが記録されている。property についても 26 characters の correction が入った。
