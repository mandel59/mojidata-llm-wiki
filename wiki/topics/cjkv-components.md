...
| 2025-10-10以前 | IRG | `IRG N2878` | `IRG N2799` の後続版。catalog entry `irg-n2878` は `IRG N2878R3` の revision lineage と、2025-10-10 付けの `IRG N2890` が `IRG N2878` への feedback であることを provenance とする derived entry である。表題は `Updated proposal to encode CJK Unified Ideographs Components`。 |
...
| 2025-10 | IRG | `IRG N2878R` | catalog entry `irg-n2878r` は IRG #66 agenda item 4.19 のリンクと IRG #65 Recommendation M65.23 を provenance とする derived entry である。M65.23 は Jianzi Musical Notation Components 121 characters と CJK Unified Ideographs Components 367 characters の two-block 構成を review し、source references、property data、non-G glyph の finalization を求めた。 |
| 2025-12-19期限 | IRG | `IRG N2878R2` | catalog entry `irg-n2878r2` は M65.23 の Action Item と `IRG N2917` の feedback target を provenance とする derived entry である。M65.23 は authors に final attribute data と final representative glyphs を含む `R2` version を 2025-12-19 までに IRG へ提出するよう求めた。 |
| 2026-03-02 | IRG | `IRG N2917` | Henry Chan が `IRG N2878R2` に対して、variant を component として符号化すべきでない点、HCP source reference、radical data などを feedback する。この feedback は `irg-n2878r2` の derived entry の provenance にもなっている。 |
...
`IRG N2878R` は IRG #66 agenda item 4.19 に直接リンクがあり、IRG #65 Recommendation M65.23 でも review 対象として扱われる。catalog entry `irg-n2878r` はこの 2 件を provenance とし、判明している document URL を持つ derived entry である。`IRG N2878R2` は M65.23 の Action Item が要求した次版で、`IRG N2917` が feedback target として明示しているため、catalog entry `irg-n2878r2` はこの 2 件を provenance とする。
