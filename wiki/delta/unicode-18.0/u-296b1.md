...
title: "Unicode 18.0 glyph delta: U+296B1 𩚱"
description: "U+296B1 𩚱のUnicode 18.0における字形更新、その根拠と審議経緯を整理する。"
slug: u-296b1
codepoint: "U+296B1"
character: "𩚱"
unicode_version: "18.0"
change_type: "KP-source horizontal extension"
...
Unicode 18.0 delta chartではExtension BのU+296B1が**KP-source horizontal extension**の対象になっている。`KP1-8852`とKP-source representative glyphを追加する。

## 変更内容

`KP1-8852`とKP-source representative glyphを追加する。

## Unihan dataの差分

| Property | Unicode 17.0 | Unicode 18.0 beta |
| --- | --- | --- |
| `kIRG_KPSource` | `—` | `KP1-8852` |

表はmojidataが固定しているUnicode 17.0 UnihanとUnicode 18.0 beta Unihanの比較である。値が不変でもchart fontの字形変更は別途存在しうる。

## 経緯

UTC \#187のCJK & Unihan reviewを経てUnicode 18.0のUnihan dataとchartに追加された。

## 影響

- plain textのcode pointはU+296B1のままであり、文字列の置換を要求する変更ではない。
