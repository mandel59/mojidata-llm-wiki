...
title: "Unicode 18.0 glyph delta: U+2FA08 䯎"
description: "U+2FA08 䯎のUnicode 18.0における字形更新、その根拠と審議経緯を整理する。"
slug: u-2fa08
codepoint: "U+2FA08"
character: "䯎"
...
Unicode 18.0 delta chartではCompatibility Ideographs SupplementのU+2FA08が**T-source mapping exchange**の対象になっている。T-sourceを`T7-2B55`から`T3-4F78`へ移し、U+4BCEとの割当てを交換する。

## 変更内容

T-sourceを`T7-2B55`から`T3-4F78`へ移し、U+4BCEとの割当てを交換する。

## Unihan dataの差分

| Property | Unicode 17.0 | Unicode 18.0 beta |
| --- | --- | --- |
| `kIRG_TSource` | `T7-2B55` | `T3-4F78` |

表はmojidataが固定しているUnicode 17.0 UnihanとUnicode 18.0 beta Unihanの比較である。値が不変でもchart fontの字形変更は別途存在しうる。

## 経緯

互換漢字側を含むsource reference交換であり、IRG N2861とUTC \#187は二字を一組として扱った。

## 影響

- plain textのcode pointはU+2FA08のままであり、文字列の置換を要求する変更ではない。
