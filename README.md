...
UTC、WG2、IRG の公開 Document Registry と Unicode Public Review Issues (PRI) を材料に、Unicode 関連の標準化議論を追うための LLM-Wiki です。
...
- `catalog/` はコミット対象の目録です。Document Registry と PRI から抽出した JSONL を置きます。
...
```sh
uv run python tools/sync_registries.py --registry all
uv run python tools/check_catalog.py
```
...
catalog/registries/pri/documents.jsonl
...
```sh
uv run python tools/fetch_documents.py --registry utc --grep "UTC #187"
...
PDF の本文抽出には project dependency の `pypdf` を使います。抽出テキストも `.cache/` 配下に置くため Git には入りません。

```sh
uv run python tools/extract_pdf_text.py .cache/unicode-docs/wg2/*/*.pdf
```
